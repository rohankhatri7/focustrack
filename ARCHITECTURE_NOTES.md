...
- `models.py` shows inheritance: `Task`, `Category`, and `Reminder` all inherit from `BaseModel`, which provides the `id` field and helper methods. Each model can override `summary()` so the same method name produces model-specific output (polymorphism).
- `BaseManager` holds shared session logic; `TaskManager` and `CategoryManager` inherit from it to avoid duplicating commit and query helpers. This is another inheritance example.
